...
def SinGAN_generate(Gs,Zs,reals,NoiseAmp,opt,in_s=None,scale_v=1,scale_h=1,n=0,gen_start_scale=0,num_samples=50):
    ...
    if in_s is None:
        in_s = jnp.zeros(reals[0].shape)
    images_cur = []
    for G,Z_opt,noise_amp in zip(Gs,Zs,NoiseAmp):
        pad1 = int(((opt.kernel_size-1)*opt.num_layer)/2)
        m = lambda arr: (jnp.pad(arr, ((0,0), (0,0), (pad1,pad1), (pad1,pad1))))
        ...
        for i in range(0,num_samples,1):
            if n == 0:
                z_curr, opt.PRNGKey  = functions.generate_noise(opt.PRNGKey, [1,nzx,nzy])
                
                z_curr = jnp.tile(z_curr, [1,3,1,1])
                z_curr = m(z_curr)
            else:
                z_curr, opt.PRNGKey  = functions.generate_noise(opt.PRNGKey, [opt.nc_z,nzx,nzy])
                z_curr = m(z_curr)
            ...
            if images_prev == []:
                ...
                I_prev = images_prev[i]
                I_prev = imresize(I_prev,1/opt.scale_factor, opt.nc_im)
                if opt.mode != "SR":
                    I_prev = I_prev[:, :, 0:round(scale_v * reals[n].shape[2]), 0:round(scale_h * reals[n].shape[3])]
                    I_prev = m(I_prev)
                    I_prev = I_prev[:,:,0:z_curr.shape[2],0:z_curr.shape[3]]
                    I_prev = functions.upsampling(I_prev,scale_v,scale_h)
                else:
                    I_prev = m(I_prev)
